...
class Settings(BaseSettings):
    ...
    github_app_private_key_path: str = Field(
        default="", description="Path to GitHub App private key PEM file (local dev)"
    )
    github_app_private_key: str = Field(
        default="", description="GitHub App private key PEM content (production — injected from AWS Secrets Manager)"
    )
    github_token: str = Field(
        default="", description="GitHub PAT fallback for local testing only. Not used in production."
    )
    ...
    llm_provider: str = Field(
        default="groq", description="Default LLM provider: gemini | openai | anthropic | groq | bedrock | ollama"
    )
    ...
    embedding_model: str = Field(
        default="all-MiniLM-L6-v2", description="Embedding model for Qdrant indexing (sentence-transformers)"
    )
    ...
    max_diff_size: int = Field(
        default=50000, description="Max cumulative diff size in chars. Files exceeding this are skipped individually."
    )
    prd_file_path: str = Field(
        default="", description="Path to Product Requirements Document. Passed to LLM for product-aware review."
    )

    # ── Monitoring Integrations ─────────────────────
    grafana_url: str = Field(
        default="", description="Grafana server URL (user-provided, e.g., https://grafana.example.com)"
    )
    ...
    datadog_host: str = Field(
        default="api.datadoghq.com", description="Datadog API host (e.g., api.ap1.datadoghq.com for Asia Pacific)"
    )
    ...
    simhash_similarity_threshold: int = Field(
        default=25, description="SimHash Hamming distance threshold for index reuse"
    )
    ...
    # ── API Authentication ────────────────────────────────────────────────
    prism_api_key: str = Field(
        default="",
        description="API key for authenticating internal endpoints (/test-review, /metrics, /dashboards, /alerts). Leave empty to allow unauthenticated access (dev only).",
    )
